...
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height) {
    // Máscaras de Sobel para detectar bordes en la imagen
    int Gx[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
    int Gy[3][3] = {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};

    // Paralelización del bucle utilizando OpenMP.
    // La directiva `parallel for collapse(2)` se usa para distribuir las iteraciones
    // de ambos bucles (y, x) entre los hilos, lo que maximiza el uso de la CPU.
    #pragma omp parallel for collapse(2) shared(imagen, imagenProcesada)
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            int sumX = 0, sumY = 0;

            // Aplicar las máscaras de Sobel (Gx y Gy) para calcular la derivada
            ...
            // Calcular la magnitud del gradiente, que indica la intensidad del borde.
            // La magnitud se normaliza a 255 (escala de grises).
